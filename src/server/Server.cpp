...
void Server::handleEvents(PollIterator& it, size_t i) {
    ...
    switch (client_fd) {
        case -1: {
            std::cerr << logEvent("[ACCEPT] Could not Accept Connection") << strerror(errno) << END
                      << std::endl;
            break;
        }
        default: {
...
void Server::handleClient(PollIterator& it) {
    ...
    switch (bytes) {
        ...
        case -1: {
            perror("recv");
            std::cerr << logEvent("[RECV] Error Receiving Data") << strerror(errno) << END
                      << std::endl;
            closeSocket(it);
            break;
        }
...
void Server::sendResponse(std::string& str_buffer, int sender_fd, char* buf) {
    ...
    if (isChunked == false) {
        ...
        if (send(sender_fd, buffer, MAX_SEND, 0) == -1) {
            std::cerr << logEvent("[SEND] Error Sending Data") << strerror(errno) << END
                      << std::endl;
        }
...
void Server::run() {
    for (size_t i = 0; i < sockets.size(); i++)
        std::cout << YELLOW
                  << logEvent("Server is listening on: " + sockets.at(i).getHostName() + "\n")
                  << END << std::endl;
