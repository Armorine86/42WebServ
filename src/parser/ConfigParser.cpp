...
ConfigParser::ConfigParser(const std::string& file_path) { parseFile(file_path); }

server_info ConfigParser::getServersInfos(const int& index) { return servers_config.at(index); }
...
void ConfigParser::fillLocationFields(StringVector vec, location_info& fields,
